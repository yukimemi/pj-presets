...
| `denops.toml` | [`pj-base`](https://github.com/yukimemi/pj-base) + [`pj-denops`](https://github.com/yukimemi/pj-denops) | **Denops plugins** (3 OS test matrix with real Vim/Neovim). Two layers only — Deno is compiled/interpreted at runtime, so there is no toolchain/build layer between them. Minor/patch Deno / JSR / GHA deps merge themselves via Renovate's platform automerge and GitHub's native auto-merge; the repo needs `allow_auto_merge` plus `ci (ubuntu-latest)` as a required status check. |
| `deno.toml` | [`pj-base`](https://github.com/yukimemi/pj-base) + [`pj-deno`](https://github.com/yukimemi/pj-deno) | **Plain Deno projects** (apps, CLIs, scrapers, cron jobs, JSR libraries). Two layers only — Deno ships its own toolchain, so there is no toolchain/build layer between them. CI is one `deno task ci` over a configurable OS matrix (`deno.os_matrix` in `.kata/vars.toml`). Use `denops.toml` instead for editor plugins. |
